...
const PAGE_URL = `${APP_ROOT}/carriers/1`;
...
async function main() {
  ...
  try {
    ...
    await page.screenshot({
      path: path.join(process.cwd(), "tmp", "carrier-card-main.png"),
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: path.join(process.cwd(), "tmp", "carrier-card-address.png"),
      fullPage: true,
    });

    await page.getByRole("tab", { name: "Tracking & service" }).click();
    await page.waitForTimeout(150);
    const trackingLabels = await activeLabels(page);
    await page.screenshot({
      path: path.join(process.cwd(), "tmp", "carrier-card-tracking.png"),
      fullPage: true,
    });

    const result = { pageState, mainLabels, addressLabels, trackingLabels };
    const outPath = path.join(process.cwd(), "tmp", "carrier-page-verification.json");
