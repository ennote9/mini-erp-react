...
const PAGE_URL = `${APP_ROOT}/customers/1`;
...
async function main() {
  ...
  try {
    ...
    const mainState = await page.evaluate(() => ({
      ...
      labels: Array.from(document.querySelectorAll('[role="tabpanel"][data-state="active"] label')).map((el) =>
        (el.textContent || "").trim(),
      ),
    }));
    await page.screenshot({ path: path.join(process.cwd(), "tmp", "customer-card-main.png"), fullPage: true });

    await page.getByRole("tab", { name: "Default delivery" }).click();
    ...
    const deliveryState = await page.evaluate(() => ({
      ...
      hint: Array.from(document.querySelectorAll('[role="tabpanel"][data-state="active"] p'))
        .map((el) => (el.textContent || "").trim())
        .find((text) => text.length > 0) ?? null,
    }));
    await page.screenshot({ path: path.join(process.cwd(), "tmp", "customer-card-delivery.png"), fullPage: true });

    const result = { mainState, deliveryState };
    const outPath = path.join(process.cwd(), "tmp", "customer-page-verification.json");
