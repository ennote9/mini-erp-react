...
const OUT_PATH = path.join(__dirname, 'receipts-shipments-status-block-removed.json');
const routes = ['/receipts', '/shipments'];
...
async function main() {
  ...
  for (const route of routes) {
    ...
    const state = await page.evaluate(() => {
      ...
      const forbidden = ['Все', 'Черновик', 'Проведён', 'Проведен', 'Реверс', 'Отменён', 'Отменен'];
