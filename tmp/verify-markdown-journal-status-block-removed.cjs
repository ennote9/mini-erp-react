...
const OUT_PATH = path.join(__dirname, 'markdown-journal-status-block-removed.json');
...
async function main() {
  ...
  const state = await page.evaluate(() => {
    ...
    const forbidden = ['Все статусы', 'Черновик', 'Проведён', 'Проведен', 'Отменён', 'Отменен'];
    ...
    return {
      ...
      viewTabsVisible: controlsText.some((text) => text.includes('Журналы')) || controlsText.some((text) => text.includes('Коды')),
    };
  });

  const screenshotPath = path.join(__dirname, 'markdown-journal-status-block-removed.png');
