...
import { buildReceiptsTanstackColumns } from "./receiptsTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<ReceiptListRow>): number | null {
...
const RECEIPTS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type ReceiptsTanstackTableProps = {
  rows: ReceiptListRow[];
  ...
  onRowClick: (row: ReceiptListRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function ReceiptsTanstackTable(props: ReceiptsTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [
      RECEIPTS_TABLE_SELECT_COLUMN_ID,
      ...columnOrder.filter((id) => id !== RECEIPTS_TABLE_SELECT_COLUMN_ID),
    ],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<ReceiptListRow, unknown> = {
      id: RECEIPTS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildReceiptsTanstackColumns({ schema, t, formatDate })];
  }, [schema, t, formatDate]);
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === RECEIPTS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === RECEIPTS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
