...
import { buildEmployeesTanstackColumns } from "./employeesTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<EmployeeListRow>): number | null {
...
const EMPLOYEES_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type EmployeesTanstackTableProps = {
...
export function EmployeesTanstackTable(props: EmployeesTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [EMPLOYEES_TABLE_SELECT_COLUMN_ID, ...columnOrder.filter((id) => id !== EMPLOYEES_TABLE_SELECT_COLUMN_ID)],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<EmployeeListRow, unknown> = {
      id: EMPLOYEES_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
      ...
      cell: ({ row }) => (
        ...
        >
          <Checkbox
            aria-label={t("doc.list.rowSelectRowAria", { code: row.original.employeeCode })}
    ...
    return [selectColumn, ...buildEmployeesTanstackColumns({ schema, t })];
  }, [schema, t]);
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === EMPLOYEES_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === EMPLOYEES_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
