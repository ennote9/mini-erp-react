...
import { buildShipmentsTanstackColumns } from "./shipmentsTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<ShipmentListRow>): number | null {
...
const SHIPMENTS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type ShipmentsTanstackTableProps = {
  ...
  onRowClick: (row: ShipmentListRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function ShipmentsTanstackTable(props: ShipmentsTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [
      SHIPMENTS_TABLE_SELECT_COLUMN_ID,
      ...columnOrder.filter((id) => id !== SHIPMENTS_TABLE_SELECT_COLUMN_ID),
    ],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<ShipmentListRow, unknown> = {
      id: SHIPMENTS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildShipmentsTanstackColumns({ schema, t, formatDate })];
  }, [schema, t, formatDate]);
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === SHIPMENTS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === SHIPMENTS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
