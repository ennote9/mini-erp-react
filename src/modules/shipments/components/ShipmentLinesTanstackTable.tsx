...
import { SHIPMENT_LINES_TANSTACK_SELECT_COLUMN_ID } from "../shipmentLinesTanstackColumns";
...
export type ShipmentLinesTanstackTableProps<TData> = {
  rows: TData[];
...
export function ShipmentLinesTanstackTable<TData>(props: ShipmentLinesTanstackTableProps<TData>) {
  const {
    rows,
    dataColumns,
    getRowId,
  ...
  const columns = useMemo(() => {
    ...
    const selectColumn: ColumnDef<TData, unknown> = {
      id: SHIPMENT_LINES_TANSTACK_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            className="flex w-full items-center justify-center"
            data-shipment-line-select-cell
            onClick={(event) => event.stopPropagation()}
            onPointerDown={(event) => event.stopPropagation()}
          >
            <Checkbox
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === SHIPMENT_LINES_TANSTACK_SELECT_COLUMN_ID;
                  ...
                  return (
                      ...
                      onClick={
                        enableRowSelection
                          ? (event) => {
                              ...
                              if (target.closest("[data-shipment-line-select-cell]")) return;
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === SHIPMENT_LINES_TANSTACK_SELECT_COLUMN_ID;
