...
import type { CustomersTableColumnSchema } from "./customersTableSchema";
import { buildCustomersTanstackColumns } from "./customersTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<CustomerListRow>): number | null {
...
const CUSTOMERS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type CustomersTanstackTableProps = {
...
export function CustomersTanstackTable(props: CustomersTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [CUSTOMERS_TABLE_SELECT_COLUMN_ID, ...columnOrder.filter((id) => id !== CUSTOMERS_TABLE_SELECT_COLUMN_ID)],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<CustomerListRow, unknown> = {
      id: CUSTOMERS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildCustomersTanstackColumns({ schema, t, formatMoney })];
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === CUSTOMERS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === CUSTOMERS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
