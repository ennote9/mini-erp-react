...
export type CustomersExportRow = {
  no: number;
  code: string;
  name: string;
  phone: string;
...
const TABLE_NAME_BASE = "CustomersTable";
...
function addSheet(workbook: Workbook, rows: CustomersExportRow[], labels: ExcelListSheetLabels): void {
  ...
  const tableRows = rows.map((r) => [r.no, r.code, r.name, r.phone, r.email, r.active]);
  sheet.addTable({
  ...
  for (let c = 0; c < COLUMN_HEADERS.length; c++) {
    const valueLengths = rows.map((r) => cellLen([r.no, r.code, r.name, r.phone, r.email, r.active][c]));
...
export async function buildCustomersListXlsxBuffer(
  rows: CustomersExportRow[],
