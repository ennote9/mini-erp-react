...
export type SuppliersTableFilterKind = "text" | "number" | "boolean" | "none";
export type SuppliersTableFormatKind = "none" | "yes-no" | "optional-text" | "payment-terms-days";

export type SuppliersTableColumnSchema = {
  id: string;
  label: string;
  accessorKey?: keyof SupplierListRow;
  ...
  filterKind: SuppliersTableFilterKind;
  formatKind?: SuppliersTableFormatKind;
  cellDataType?: boolean;
};

type BuildSuppliersTableSchemaInput = {
  t: TFunction;
};

export function buildSuppliersTableSchema(input: BuildSuppliersTableSchemaInput): SuppliersTableColumnSchema[] {
  const { t } = input;
  return [
    {
      id: "lineNo",
