/**
 * Suppliers master list — TanStack Table renderer aligned with the Items list architecture.
 */
import {
  functionalUpdate,
...
import { buildSuppliersListViewCatalog } from "../suppliersListViewFieldCatalog";
import { buildSupplierListRows, type SupplierListRow } from "../supplierListRowModel";
import { buildSuppliersTableSchema, type SuppliersTableColumnSchema } from "../suppliersTableSchema";
import { buildSuppliersTableListViewState } from "../suppliersListViewState";
import { formatSuppliersTableValue } from "../suppliersTanstackColumns";
import { SuppliersTanstackTable } from "../SuppliersTanstackTable";
import { ItemsHeaderFilterPanel } from "@/modules/items/ItemsHeaderFilterPanel";

const COLUMN_SIZING_STORAGE_KEY = "mini-erp:suppliers:tanstack:columnSizing:v1";
...
function suppliersExportDiagLog(enabled: boolean, message: string, detail?: unknown) {
  if (!enabled) return;
  console.info("[suppliers-export]", message, detail ?? "");
}
...
function downloadBufferInBrowser(
  ...
  suppliersExportDiagLog(exportDiag, "browser download helper entered", { downloadFilename, mimeType });
  ...
  suppliersExportDiagLog(exportDiag, "browser download helper completed", { downloadFilename });
}
...
function coerceWriteBufferResult(data: unknown): ArrayBuffer {
  ...
  throw new Error(`[suppliers-export] unexpected workbook buffer type: ${Object.prototype.toString.call(data)}`);
}
...
export function SuppliersListPage() {
  ...
  const listRows = useMemo(() => buildSupplierListRows(filteredSuppliers), [filteredSuppliers]);

  const suppliersListViewCatalog = useMemo(
    () =>
      buildSuppliersListViewCatalog({
        t,
        formatMoney,
      }),
    [t, locale, formatMoney, appReadRevision],
  );
  const suppliersTableSchema = useMemo(() => buildSuppliersTableSchema({ t }), [t, locale, appReadRevision]);

  useEffect(() => {
    setColumnSizing((current) => {
      const next = sanitizeColumnSizing(current, suppliersTableSchema);
  ...
  }, [suppliersTableSchema]);

  const baseColumnDefs = suppliersListViewCatalog.columnDefs;
  const supplierFieldRegistry = suppliersListViewCatalog.fieldRegistry;
  const supplierColumnFilterConfigs = suppliersListViewCatalog.filterConfigs;

  const displayRowsWithQueryFilters = useMemo(
    () => applyListViewColumnFilters(listRows, columnFilterModel, supplierColumnFilterConfigs),
    [listRows, columnFilterModel, supplierColumnFilterConfigs],
  );
  ...
  } = useListViewColumnSettings<SupplierListRow>({
    pageKey: "suppliers",
    entityType: "suppliers",
    baseColumnDefs,
    fieldRegistry: supplierFieldRegistry,
  ...
  const resolveDeepSortValue = useCallback(
    (row: SupplierListRow, fieldKey: string): unknown => {
  ...
  const displayRowsWithDeepFilters = useMemo(
    () => applyListViewColumnFilters(displayRowsWithQueryFilters, deepFilterModel, supplierColumnFilterConfigs),
  ...
  const neutralListViewState = useMemo(
    () =>
      buildSuppliersTableListViewState({
        definition: columnSettingsDefinition,
        columnFilterModel,
        sortModel: effectiveSortModel,
        personalViews: columnSettingsPersonalViews,
  ...
  const fallbackColumnVisibility = useMemo<VisibilityState>(
    () =>
      Object.fromEntries(
        suppliersTableSchema.map((column) => [column.id, column.lockedVisible ? true : column.defaultVisible]),
      ),
    [suppliersTableSchema],
  );
  const fallbackColumnOrder = useMemo(
    () => suppliersTableSchema.map((column) => column.id),
    [suppliersTableSchema],
  );
  ...
  const activeHeaderFilterConfig =
    activeHeaderFilterField != null ? supplierColumnFilterConfigs[activeHeaderFilterField] : undefined;
  ...
  const handleColumnSizingChange = useCallback(
    (updater: ColumnSizingState | ((old: ColumnSizingState) => ColumnSizingState)) => {
      setColumnSizing((current) => {
        const next = sanitizeColumnSizing(functionalUpdate(updater, current), suppliersTableSchema);
        return next;
      });
    },
    [suppliersTableSchema],
  ...
  const visibleSchemaColumns = useMemo(() => {
    const schemaById = new Map(suppliersTableSchema.map((column) => [column.id, column]));
    return tableColumnOrder
      .map((id) => schemaById.get(id))
      .filter((column): column is SuppliersTableColumnSchema => Boolean(column))
      .filter((column) => tableColumnVisibility[column.id] !== false);
  }, [suppliersTableSchema, tableColumnOrder, tableColumnVisibility]);

  const buildExportPayloadForRows = useCallback(
    (rows: SupplierListRow[]): { headers: string[]; rows: Array<Array<string | number>> } => {
  ...
  const runExportWithSaveAs = useCallback(
    async (
      ...
      suppliersExportDiagLog(exportDiag, "runtime branch chosen", { isTauriRuntime: tauri });

      if (!tauri) {
        suppliersExportDiagLog(exportDiag, "native save path skipped (browser/dev)");
        try {
          suppliersExportDiagLog(exportDiag, "real XLSX buffer build started (browser path)");
          const raw = await buildBuffer();
          suppliersExportDiagLog(exportDiag, "real XLSX buffer build finished (browser path)", {
          ...
          suppliersExportDiagLog(exportDiag, "catch/fallback entered (browser path)", { err: String(err) });
        }
        return;
      }

      suppliersExportDiagLog(exportDiag, "native save path entered (Tauri)");
      try {
        const path = await save({
          defaultPath: generatedFilename,
          filters: [{ name: t("doc.page.excelFilterName"), extensions: ["xlsx"] }],
        });
        suppliersExportDiagLog(exportDiag, "save() returned", { path: path ?? null });
        suppliersExportDiagLog(exportDiag, "real XLSX buffer build started (Tauri path)");
        const raw = await buildBuffer();
        suppliersExportDiagLog(exportDiag, "real XLSX buffer build finished (Tauri path)", {
          byteLength: raw instanceof ArrayBuffer ? raw.byteLength : (raw as Uint8Array).byteLength,
        });
        const buffer = coerceWriteBufferResult(raw);

        if (path == null) {
          suppliersExportDiagLog(exportDiag, "save() null — browser download fallback");
          downloadBufferInBrowser(buffer, generatedFilename, fallbackMime, exportDiag);
          return;
        }

        suppliersExportDiagLog(exportDiag, "native write path entered", { path });
        ...
        suppliersExportDiagLog(exportDiag, "export success reached (Tauri write)", { filename });
      } catch (err) {
        console.error("Export failed", err);
        suppliersExportDiagLog(exportDiag, "catch/fallback entered (Tauri path)", { err: String(err) });
        try {
          const raw = await buildBuffer();
          const buffer = coerceWriteBufferResult(raw);
          downloadBufferInBrowser(buffer, generatedFilename, fallbackMime, exportDiag);
  ...
  const handleExportCurrentView = useCallback(async () => {
    ...
    if (exportDiag) {
      suppliersExportDiagLog(exportDiag, "export click entered", { exportDiag: true });
      downloadBufferInBrowser(
        "suppliers-export delivery probe\n",
        "suppliers-export-probe.txt",
        "text/plain",
        exportDiag,
      );
    }
    ...
    suppliersExportDiagLog(exportDiag, "real export payload built", {
      headerCount: payload.headers.length,
      rowCount: payload.rows.length,
    });

    if (payload.headers.length === 0) {
      suppliersExportDiagLog(exportDiag, "zero visible columns — placeholder XLSX");
  ...
  const handleExportSelectedRows = useCallback(async () => {
    ...
    suppliersExportDiagLog(exportDiag, "selected export payload built", {
      headerCount: payload.headers.length,
  ...
  const listContent = (
    <>
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <SuppliersTanstackTable
          ...
          onRowClick={(row) => {
            ...
            navigate(appendReturnTo(`/suppliers/${row.id}`, currentReturnTo));
          }}
          t={t}
      ...
      <ListViewColumnSettingsModal
        ...
        registry={columnSettingsRegistry}
        filterConfigs={supplierColumnFilterConfigs as Record<string, ListViewColumnFilterConfig<unknown>>}
        ...
        <div className="list-page__controls-stack flex w-full min-w-0 flex-wrap items-center justify-between gap-x-4 gap-y-2">
          ...
          <div className="list-page__toolbar-actions-cluster flex max-w-full min-w-0 flex-wrap items-center justify-end gap-2">
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    console.error("[suppliers-export] export failed", err);
                  }
                }}
              >
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          console.error("[suppliers-export] selected export failed", err);
                        } finally {
                          setExportOpen(false);
                        }
                      }}
            ...
            <Button
              ...
              onClick={() => navigate(appendReturnTo("/suppliers/new", currentReturnTo))}
            >
              <svg
                className="h-3 w-3"
                viewBox="0 0 24 24"
