...
import { buildSuppliersTanstackColumns } from "./suppliersTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<SupplierListRow>): number | null {
...
const SUPPLIERS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type SuppliersTanstackTableProps = {
  ...
  onRowClick: (row: SupplierListRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function SuppliersTanstackTable(props: SuppliersTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [SUPPLIERS_TABLE_SELECT_COLUMN_ID, ...columnOrder.filter((id) => id !== SUPPLIERS_TABLE_SELECT_COLUMN_ID)],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<SupplierListRow, unknown> = {
      id: SUPPLIERS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildSuppliersTanstackColumns({ schema, t, formatMoney })];
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === SUPPLIERS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === SUPPLIERS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
