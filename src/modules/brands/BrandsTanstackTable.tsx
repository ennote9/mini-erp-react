...
import { buildBrandsTanstackColumns } from "./brandsTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<BrandListRow>): number | null {
...
const BRANDS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type BrandsTanstackTableProps = {
  rows: BrandListRow[];
  ...
  onRowClick: (row: BrandListRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function BrandsTanstackTable(props: BrandsTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [BRANDS_TABLE_SELECT_COLUMN_ID, ...columnOrder.filter((id) => id !== BRANDS_TABLE_SELECT_COLUMN_ID)],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<BrandListRow, unknown> = {
      id: BRANDS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildBrandsTanstackColumns({ schema, t, formatMoney })];
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === BRANDS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === BRANDS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
