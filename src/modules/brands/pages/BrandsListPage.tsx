/**
 * Brands master list — TanStack Table renderer aligned with the Items list architecture.
 */
import {
  functionalUpdate,
...
import { buildBrandsListViewCatalog } from "../brandsListViewFieldCatalog";
import { buildBrandListRows, type BrandListRow } from "../brandListRowModel";
import { buildBrandsTableSchema, type BrandsTableColumnSchema } from "../brandsTableSchema";
import { buildBrandsTableListViewState } from "../brandsListViewState";
import { formatBrandsTableValue } from "../brandsTanstackColumns";
import { BrandsTanstackTable } from "../BrandsTanstackTable";
import { ItemsHeaderFilterPanel } from "@/modules/items/ItemsHeaderFilterPanel";

const COLUMN_SIZING_STORAGE_KEY = "mini-erp:brands:tanstack:columnSizing:v1";
...
function brandsExportDiagLog(enabled: boolean, message: string, detail?: unknown) {
  if (!enabled) return;
  console.info("[brands-export]", message, detail ?? "");
}
...
function downloadBufferInBrowser(
  ...
  brandsExportDiagLog(exportDiag, "browser download helper entered", { downloadFilename, mimeType });
  ...
  brandsExportDiagLog(exportDiag, "browser download helper completed", { downloadFilename });
}
...
function coerceWriteBufferResult(data: unknown): ArrayBuffer {
  ...
  throw new Error(`[brands-export] unexpected workbook buffer type: ${Object.prototype.toString.call(data)}`);
}
...
export function BrandsListPage() {
  ...
  const filteredBrands = useMemo(() => {
    return brandRepository.search(searchQuery);
  }, [searchQuery, appReadRevision]);

  const listRows = useMemo(() => buildBrandListRows(filteredBrands), [filteredBrands]);

  const brandsListViewCatalog = useMemo(
    () =>
      buildBrandsListViewCatalog({
        t,
        formatMoney,
      }),
    [t, locale, formatMoney, appReadRevision],
  );
  const brandsTableSchema = useMemo(() => buildBrandsTableSchema({ t }), [t, locale, appReadRevision]);

  useEffect(() => {
    setColumnSizing((current) => {
      const next = sanitizeColumnSizing(current, brandsTableSchema);
  ...
  const baseColumnDefs = brandsListViewCatalog.columnDefs;
  const brandFieldRegistry = brandsListViewCatalog.fieldRegistry;
  const brandColumnFilterConfigs = brandsListViewCatalog.filterConfigs;

  const displayRowsWithQueryFilters = useMemo(
    () => applyListViewColumnFilters(listRows, columnFilterModel, brandColumnFilterConfigs),
    [listRows, columnFilterModel, brandColumnFilterConfigs],
  );
  ...
  } = useListViewColumnSettings<BrandListRow>({
    pageKey: "brands",
    entityType: "brands",
    baseColumnDefs,
    fieldRegistry: brandFieldRegistry,
  ...
  const resolveDeepSortValue = useCallback(
    (row: BrandListRow, fieldKey: string): unknown => {
      const config = brandColumnFilterConfigs[fieldKey];
      if (config?.getValue) return config.getValue(row);
      return (row as unknown as Record<string, unknown>)[fieldKey];
    },
    [brandColumnFilterConfigs],
  );

  const displayRowsWithDeepFilters = useMemo(
    () => applyListViewColumnFilters(displayRowsWithQueryFilters, deepFilterModel, brandColumnFilterConfigs),
    [displayRowsWithQueryFilters, deepFilterModel, brandColumnFilterConfigs],
  );
  ...
  const fallbackColumnVisibility = useMemo<VisibilityState>(
    () =>
      Object.fromEntries(
        brandsTableSchema.map((column) => [column.id, column.lockedVisible ? true : column.defaultVisible]),
      ),
    [brandsTableSchema],
  );
  const fallbackColumnOrder = useMemo(
    () => brandsTableSchema.map((column) => column.id),
    [brandsTableSchema],
  );
  ...
  const activeHeaderFilterConfig =
    activeHeaderFilterField != null ? brandColumnFilterConfigs[activeHeaderFilterField] : undefined;
  ...
  const handleColumnSizingChange = useCallback(
    (updater: ColumnSizingState | ((old: ColumnSizingState) => ColumnSizingState)) => {
      setColumnSizing((current) => {
        const next = sanitizeColumnSizing(functionalUpdate(updater, current), brandsTableSchema);
        return next;
      });
    },
    [brandsTableSchema],
  ...
  const visibleSchemaColumns = useMemo(() => {
    const schemaById = new Map(brandsTableSchema.map((column) => [column.id, column]));
    return tableColumnOrder
      .map((id) => schemaById.get(id))
      .filter((column): column is BrandsTableColumnSchema => Boolean(column))
      .filter((column) => tableColumnVisibility[column.id] !== false);
  }, [brandsTableSchema, tableColumnOrder, tableColumnVisibility]);

  const buildExportPayloadForRows = useCallback(
    (rows: BrandListRow[]): { headers: string[]; rows: Array<Array<string | number>> } => {
  ...
  const runExportWithSaveAs = useCallback(
    async (
      ...
      brandsExportDiagLog(exportDiag, "runtime branch chosen", { isTauriRuntime: tauri });

      if (!tauri) {
        brandsExportDiagLog(exportDiag, "native save path skipped (browser/dev)");
        try {
          brandsExportDiagLog(exportDiag, "real XLSX buffer build started (browser path)");
          const raw = await buildBuffer();
          brandsExportDiagLog(exportDiag, "real XLSX buffer build finished (browser path)", {
          ...
          brandsExportDiagLog(exportDiag, "catch/fallback entered (browser path)", { err: String(err) });
        }
        return;
      }

      brandsExportDiagLog(exportDiag, "native save path entered (Tauri)");
      try {
        const path = await save({
          defaultPath: generatedFilename,
          filters: [{ name: t("doc.page.excelFilterName"), extensions: ["xlsx"] }],
        });
        brandsExportDiagLog(exportDiag, "save() returned", { path: path ?? null });
        brandsExportDiagLog(exportDiag, "real XLSX buffer build started (Tauri path)");
        const raw = await buildBuffer();
        brandsExportDiagLog(exportDiag, "real XLSX buffer build finished (Tauri path)", {
          byteLength: raw instanceof ArrayBuffer ? raw.byteLength : (raw as Uint8Array).byteLength,
        });
        const buffer = coerceWriteBufferResult(raw);

        if (path == null) {
          brandsExportDiagLog(exportDiag, "save() null — browser download fallback");
          downloadBufferInBrowser(buffer, generatedFilename, fallbackMime, exportDiag);
          return;
        }

        brandsExportDiagLog(exportDiag, "native write path entered", { path });
        ...
        brandsExportDiagLog(exportDiag, "export success reached (Tauri write)", { filename });
      } catch (err) {
        console.error("Export failed", err);
        brandsExportDiagLog(exportDiag, "catch/fallback entered (Tauri path)", { err: String(err) });
        try {
          const raw = await buildBuffer();
          const buffer = coerceWriteBufferResult(raw);
          downloadBufferInBrowser(buffer, generatedFilename, fallbackMime, exportDiag);
  ...
  const handleExportCurrentView = useCallback(async () => {
    ...
    if (exportDiag) {
      brandsExportDiagLog(exportDiag, "export click entered", { exportDiag: true });
      downloadBufferInBrowser(
        "brands-export delivery probe\n",
        "brands-export-probe.txt",
        "text/plain",
        exportDiag,
      );
    }
    ...
    brandsExportDiagLog(exportDiag, "real export payload built", {
      headerCount: payload.headers.length,
      rowCount: payload.rows.length,
    });

    if (payload.headers.length === 0) {
      brandsExportDiagLog(exportDiag, "zero visible columns — placeholder XLSX");
  ...
  const handleExportSelectedRows = useCallback(async () => {
    ...
    brandsExportDiagLog(exportDiag, "selected export payload built", {
      headerCount: payload.headers.length,
    ...
    if (payload.headers.length === 0) {
      await runExportWithSaveAs(
        "brands-selected.xlsx",
        () =>
          buildListViewXlsxBuffer({
            sheetName: listExcelLabels.sheetName,
            headers: ["—"],
            rows: [["No visible columns. Use View settings to show at least one column, then export again."]],
            tableNameBase: "BrandsListViewSelected",
          }),
        exportDiag,
      );
      return;
  ...
  const noRowsOverlay = useMemo(
    () =>
      getListViewEmptyStateContent(
        {
          baseRowCount: brandRepository.list().length,
  ...
  const listContent = (
    <>
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <BrandsTanstackTable
          ...
          onRowClick={(row) => {
            ...
            navigate(appendReturnTo(`/brands/${row.id}`, currentReturnTo));
          }}
          t={t}
      ...
      <ListViewColumnSettingsModal
        ...
        registry={columnSettingsRegistry}
        filterConfigs={brandColumnFilterConfigs as Record<string, ListViewColumnFilterConfig<unknown>>}
      ...
      controls={
        <div className="list-page__controls-stack flex w-full min-w-0 flex-wrap items-center justify-between gap-x-4 gap-y-2">
          <ListPageSearch
            ...
            placeholder={t("ops.list.brands.searchPlaceholder")}
            value={searchQuery}
            onChange={handleSearchQueryChange}
            debounceMs={220}
            aria-label={t("ops.list.brands.searchAria")}
          ...
          <div className="list-page__toolbar-actions-cluster flex max-w-full min-w-0 flex-wrap items-center justify-end gap-2">
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    console.error("[brands-export] export failed", err);
                  }
                }}
              >
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          console.error("[brands-export] selected export failed", err);
                        } finally {
                          setExportOpen(false);
                        }
                      }}
            ...
            <Button
              ...
              onClick={() => navigate(appendReturnTo("/brands/new", currentReturnTo))}
            >
              <svg
                className="h-3 w-3"
                viewBox="0 0 24 24"
