...
import { buildWarehousesTanstackColumns } from "./warehousesTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<WarehouseListRow>): number | null {
...
const WAREHOUSES_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type WarehousesTanstackTableProps = {
  ...
  onRowClick: (row: WarehouseListRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function WarehousesTanstackTable(props: WarehousesTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [WAREHOUSES_TABLE_SELECT_COLUMN_ID, ...columnOrder.filter((id) => id !== WAREHOUSES_TABLE_SELECT_COLUMN_ID)],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<WarehouseListRow, unknown> = {
      id: WAREHOUSES_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildWarehousesTanstackColumns({ schema, t, formatMoney })];
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === WAREHOUSES_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === WAREHOUSES_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
