/**
 * Sales orders list — TanStack Table renderer aligned with the Items list architecture.
 */
import {
  functionalUpdate,
...
import { buildSalesOrdersListViewCatalog } from "../salesOrdersListViewFieldCatalog";
import { buildSalesOrderListRows, type SalesOrderListRow } from "../salesOrderListRowModel";
import { buildSalesOrdersTableSchema, type SalesOrdersTableColumnSchema } from "../salesOrdersTableSchema";
import { buildSalesOrdersTableListViewState } from "../salesOrdersListViewState";
import { formatSalesOrdersTableValue } from "../salesOrdersTanstackColumns";
import { SalesOrdersTanstackTable } from "../SalesOrdersTanstackTable";
import { ItemsHeaderFilterPanel } from "@/modules/items/ItemsHeaderFilterPanel";

const COLUMN_SIZING_STORAGE_KEY = "mini-erp:sales-orders:tanstack:columnSizing:v1";
...
function salesOrdersExportDiagLog(enabled: boolean, message: string, detail?: unknown) {
  if (!enabled) return;
  console.info("[sales-orders-export]", message, detail ?? "");
}
...
function downloadBufferInBrowser(
  ...
  salesOrdersExportDiagLog(exportDiag, "browser download helper entered", { downloadFilename, mimeType });
  ...
  salesOrdersExportDiagLog(exportDiag, "browser download helper completed", { downloadFilename });
}
...
function coerceWriteBufferResult(data: unknown): ArrayBuffer {
  ...
  throw new Error(`[sales-orders-export] unexpected workbook buffer type: ${Object.prototype.toString.call(data)}`);
}
...
function filterBySearch(rows: SalesOrderListRow[], query: string): SalesOrderListRow[] {
...
function filterByCustomerId(rows: SalesOrderListRow[], customerId: string | null): SalesOrderListRow[] {
  if (customerId == null) return rows;
  return rows.filter((r) => r.customerId === customerId);
}

function filterByWarehouseId(rows: SalesOrderListRow[], warehouseId: string | null): SalesOrderListRow[] {
  if (warehouseId == null) return rows;
  return rows.filter((r) => r.warehouseId === warehouseId);
}

function filterByCarrierId(rows: SalesOrderListRow[], carrierId: string | null): SalesOrderListRow[] {
  if (carrierId == null) return rows;
  return rows.filter((r) => (r.carrierId?.trim() ?? "") === carrierId);
}

function filterByDocumentIdSet(rows: SalesOrderListRow[], allowIds: Set<string> | null): SalesOrderListRow[] {
...
export function SalesOrdersListPage() {
  ...
  const listRows = useMemo(() => buildSalesOrderListRows(t), [t, locale, appReadRevision]);
  ...
  const customerNameEnumOptions = useMemo(
    () =>
      Array.from(new Set(listRows.map((row) => row.customerName)))
        .filter(Boolean)
  ...
  const salesOrdersListViewCatalog = useMemo(
    () =>
      buildSalesOrdersListViewCatalog({
        t,
        formatDate,
        customerNameEnumOptions,
        warehouseNameEnumOptions,
        statusEnumOptions,
      }),
    [t, locale, formatDate, customerNameEnumOptions, warehouseNameEnumOptions, statusEnumOptions],
  );
  const salesOrdersTableSchema = useMemo(() => buildSalesOrdersTableSchema({ t }), [t, locale, appReadRevision]);

  useEffect(() => {
    setColumnSizing((current) => {
      const next = sanitizeColumnSizing(current, salesOrdersTableSchema);
  ...
  }, [salesOrdersTableSchema]);

  const baseColumnDefs = salesOrdersListViewCatalog.columnDefs;
  const salesOrderFieldRegistry = salesOrdersListViewCatalog.fieldRegistry;
  const salesOrderColumnFilterConfigs = salesOrdersListViewCatalog.filterConfigs;

  const displayRowsWithQueryFilters = useMemo(
    () => applyListViewColumnFilters(filteredRows, columnFilterModel, salesOrderColumnFilterConfigs),
  ...
  } = useListViewColumnSettings<SalesOrderListRow>({
    pageKey: "sales-orders",
    entityType: "sales-orders",
    baseColumnDefs,
    fieldRegistry: salesOrderFieldRegistry,
  ...
  const resolveDeepSortValue = useCallback(
    (row: SalesOrderListRow, fieldKey: string): unknown => {
      const config = salesOrderColumnFilterConfigs[fieldKey];
  ...
  const displayRowsWithDeepFilters = useMemo(
    () => applyListViewColumnFilters(displayRowsWithQueryFilters, deepFilterModel, salesOrderColumnFilterConfigs),
    [displayRowsWithQueryFilters, deepFilterModel, salesOrderColumnFilterConfigs],
  );
  ...
  const neutralListViewState = useMemo(
    () =>
      buildSalesOrdersTableListViewState({
        definition: columnSettingsDefinition,
        columnFilterModel,
        sortModel: effectiveSortModel,
        personalViews: columnSettingsPersonalViews,
  ...
  const fallbackColumnVisibility = useMemo<VisibilityState>(
    () =>
      Object.fromEntries(
        salesOrdersTableSchema.map((column) => [column.id, column.lockedVisible ? true : column.defaultVisible]),
      ),
    [salesOrdersTableSchema],
  );
  const fallbackColumnOrder = useMemo(
    () => salesOrdersTableSchema.map((column) => column.id),
    [salesOrdersTableSchema],
  );
  ...
  const activeHeaderFilterConfig =
    activeHeaderFilterField != null ? salesOrderColumnFilterConfigs[activeHeaderFilterField] : undefined;
  ...
  const handleColumnSizingChange = useCallback(
    (updater: ColumnSizingState | ((old: ColumnSizingState) => ColumnSizingState)) => {
      setColumnSizing((current) => {
        const next = sanitizeColumnSizing(functionalUpdate(updater, current), salesOrdersTableSchema);
        return next;
      });
    },
    [salesOrdersTableSchema],
  ...
  const visibleSchemaColumns = useMemo(() => {
    const schemaById = new Map(salesOrdersTableSchema.map((column) => [column.id, column]));
    return tableColumnOrder
      .map((id) => schemaById.get(id))
      .filter((column): column is SalesOrdersTableColumnSchema => Boolean(column))
      .filter((column) => tableColumnVisibility[column.id] !== false);
  }, [salesOrdersTableSchema, tableColumnOrder, tableColumnVisibility]);

  const buildExportPayloadForRows = useCallback(
    (rows: SalesOrderListRow[]): { headers: string[]; rows: Array<Array<string | number>> } => {
  ...
  const runExportWithSaveAs = useCallback(
    async (
      ...
      salesOrdersExportDiagLog(exportDiag, "runtime branch chosen", { isTauriRuntime: tauri });

      if (!tauri) {
        salesOrdersExportDiagLog(exportDiag, "native save path skipped (browser/dev)");
        try {
          salesOrdersExportDiagLog(exportDiag, "real XLSX buffer build started (browser path)");
          const raw = await buildBuffer();
          salesOrdersExportDiagLog(exportDiag, "real XLSX buffer build finished (browser path)", {
          ...
          salesOrdersExportDiagLog(exportDiag, "catch/fallback entered (browser path)", { err: String(err) });
        }
        return;
      }

      salesOrdersExportDiagLog(exportDiag, "native save path entered (Tauri)");
      try {
        const path = await save({
          defaultPath: generatedFilename,
          filters: [{ name: t("doc.page.excelFilterName"), extensions: ["xlsx"] }],
        });
        salesOrdersExportDiagLog(exportDiag, "save() returned", { path: path ?? null });
        salesOrdersExportDiagLog(exportDiag, "real XLSX buffer build started (Tauri path)");
        const raw = await buildBuffer();
        salesOrdersExportDiagLog(exportDiag, "real XLSX buffer build finished (Tauri path)", {
          byteLength: raw instanceof ArrayBuffer ? raw.byteLength : (raw as Uint8Array).byteLength,
        });
        const buffer = coerceWriteBufferResult(raw);

        if (path == null) {
          salesOrdersExportDiagLog(exportDiag, "save() null — browser download fallback");
          downloadBufferInBrowser(buffer, generatedFilename, fallbackMime, exportDiag);
          return;
        }

        salesOrdersExportDiagLog(exportDiag, "native write path entered", { path });
        ...
        salesOrdersExportDiagLog(exportDiag, "export success reached (Tauri write)", { filename });
      } catch (err) {
        console.error("Export failed", err);
        salesOrdersExportDiagLog(exportDiag, "catch/fallback entered (Tauri path)", { err: String(err) });
        try {
          const raw = await buildBuffer();
          const buffer = coerceWriteBufferResult(raw);
          downloadBufferInBrowser(buffer, generatedFilename, fallbackMime, exportDiag);
  ...
  const handleExportCurrentView = useCallback(async () => {
    ...
    if (exportDiag) {
      salesOrdersExportDiagLog(exportDiag, "export click entered", { exportDiag: true });
      downloadBufferInBrowser(
        "sales-orders-export delivery probe\n",
        "sales-orders-export-probe.txt",
        "text/plain",
        exportDiag,
      );
    }
    ...
    salesOrdersExportDiagLog(exportDiag, "real export payload built", {
      headerCount: payload.headers.length,
      rowCount: payload.rows.length,
    });

    if (payload.headers.length === 0) {
      salesOrdersExportDiagLog(exportDiag, "zero visible columns — placeholder XLSX");
      await runExportWithSaveAs(
        ...
        () =>
          buildListViewXlsxBuffer({
            ...
            tableNameBase: "SalesOrdersListView",
          }),
        exportDiag,
      );
      return;
  ...
  const handleExportSelectedRows = useCallback(async () => {
    ...
    salesOrdersExportDiagLog(exportDiag, "selected export payload built", {
      headerCount: payload.headers.length,
    ...
    if (payload.headers.length === 0) {
      await runExportWithSaveAs(
        "sales-orders-selected.xlsx",
        () =>
          buildListViewXlsxBuffer({
            sheetName: listExcelLabels.sheetName,
            headers: ["—"],
            rows: [["No visible columns. Use View settings to show at least one column, then export again."]],
            tableNameBase: "SalesOrdersListViewSelected",
          }),
        exportDiag,
      );
      return;
  ...
  const listContent = (
    <>
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <SalesOrdersTanstackTable
          rows={displayRows}
          schema={salesOrdersTableSchema}
          ...
          onRowClick={(row) => {
            ...
            navigate(appendReturnTo(`/sales-orders/${row.id}`, currentReturnTo));
          }}
          t={t}
      ...
      <ListViewColumnSettingsModal
        ...
        registry={columnSettingsRegistry}
        filterConfigs={salesOrderColumnFilterConfigs as Record<string, ListViewColumnFilterConfig<unknown>>}
        ...
        <div className="list-page__controls-stack flex w-full min-w-0 flex-wrap items-center justify-between gap-x-4 gap-y-2">
          ...
          <div className="list-page__toolbar-actions-cluster flex max-w-full min-w-0 flex-wrap items-center justify-end gap-2">
            ...
            {carrierFilterId != null && (
              ...
              >
                <span className="shrink-0 whitespace-nowrap text-muted-foreground">{t("doc.so.carrier")}</span>
            ...
            {customerFilterId != null && (
              ...
              >
                <span className="shrink-0 whitespace-nowrap text-muted-foreground">{t("doc.columns.customer")}</span>
                <span className="min-w-0 truncate font-medium text-foreground/90" title={customerFilterLabel}>
                  {customerFilterLabel}
                </span>
                <Button
                  type="button"
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    console.error("[sales-orders-export] export failed", err);
                  }
                }}
              >
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          console.error("[sales-orders-export] selected export failed", err);
                        } finally {
                          setExportOpen(false);
                        }
                      }}
