...
export function SalesOrderCustomerInvoicePage() {
  ...
  const view = useMemo(() => {
    ...
    const payments = salesOrderPaymentRepository.listBySalesOrderId(id);
    ...
    const paymentSummary = deriveSalesOrderPaymentSummary(grandTotal, payments);
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      const result = await savePrintableDocumentPdf({
        root,
        defaultFilename: `payment-request-${snapshotDocNumber}.pdf`,
        filterName: t("doc.page.documentPdfFilterName"),
      });
      if (result) setDownloadSuccess(result);
    } catch (err) {
      console.error("Payment request PDF download failed", err);
  ...
  if (view.unavailable) {
    return (
      <div className="p-6 space-y-3">
        <p className="text-sm text-muted-foreground">{t("finance.invoiceUnavailable")}</p>
  ...
  return (
    <div className="customer-doc mx-auto w-full max-w-5xl px-5 py-6 text-foreground sm:px-8 print:max-w-none print:px-0 print:py-0">
      <div className="customer-doc__toolbar mb-5 flex flex-wrap items-center gap-2 print:hidden">
        ...
        <DocumentExportSuccessStrip success={downloadSuccess} onDismiss={() => setDownloadSuccess(null)} />
      ...
      <div ref={downloadRootRef} className="customer-doc__download-snapshot w-full">
        <header className="mb-5 border-b-2 border-border pb-4 print:border-black/30">
          ...
          <p className="mt-1 text-sm text-muted-foreground print:text-black/70">{t("finance.invoiceSubtitle")}</p>
        ...
        <section className="mb-5">
          ...
          <div className="rounded-lg border border-border bg-muted/10 px-3.5 py-2 text-sm print:border-black/22 print:bg-white">
            <dl className="grid gap-x-8 gap-y-1.5 sm:grid-cols-2">
              ...
              <div>
                <dt className="text-xs font-medium text-muted-foreground print:text-black/55">{t("doc.page.dueDate")}</dt>
                <dd className="font-semibold tabular-nums">
                  {doc.dueDate != null && doc.dueDate !== "" ? doc.dueDate : emDash}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-medium text-muted-foreground print:text-black/55">{t("doc.summary.paymentTerms")}</dt>
        ...
        <section className="mb-5">
          <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground print:text-black/65">
            {t("finance.sectionPaymentSummary")}
          </h2>
          <div className="rounded-lg border border-border bg-muted/10 px-3.5 py-2 text-sm print:border-black/22">
            <dl className="grid gap-x-8 gap-y-1 sm:grid-cols-2">
              <div>
                <dt className="text-xs font-medium text-muted-foreground print:text-black/55">{t("common.status")}</dt>
          ...
          <section className="mb-6">
            ...
            <div className="overflow-x-auto rounded-lg border border-border print:overflow-visible">
              <table className="customer-doc__table w-full border-collapse text-sm">
                <thead>
                  <tr>
                    <th className="customer-doc__th">{t("finance.paidAt")}</th>
                    <th className="customer-doc__th customer-doc__th--numeric">{t("finance.amount")}</th>
                    <th className="customer-doc__th">{t("finance.method")}</th>
                    <th className="customer-doc__th">{t("finance.reference")}</th>
                  </tr>
                </thead>
                <tbody>
                  {view.payments.map((p) => (
        ...
        <footer className="customer-doc__meta text-sm text-muted-foreground print:text-black/55">
          <span className="font-medium text-foreground/80 print:text-black/70">{t("finance.printedAt")}</span>
          {": "}
          <span className="tabular-nums">{printedAt}</span>
          <span className="mx-2 print:hidden">·</span>
          <span className="block sm:inline print:block">{t("finance.invoicePreparedFrom", { number: doc.number })}</span>
        </footer>
      </div>
    </div>
  );
