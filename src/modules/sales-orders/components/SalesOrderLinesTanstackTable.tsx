...
import { SO_LINES_TANSTACK_SELECT_COLUMN_ID } from "../salesOrderLinesTanstackColumns";
...
export type SalesOrderLinesTanstackTableProps<TData> = {
  rows: TData[];
...
export function SalesOrderLinesTanstackTable<TData>(props: SalesOrderLinesTanstackTableProps<TData>) {
  const {
    rows,
    dataColumns,
    getRowId,
  ...
  const columns = useMemo(() => {
    ...
    const selectColumn: ColumnDef<TData, unknown> = {
      id: SO_LINES_TANSTACK_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === SO_LINES_TANSTACK_SELECT_COLUMN_ID;
                  ...
                  return (
                      ...
                      onClick={
                        enableRowSelection
                          ? (event) => {
                              ...
                              if (target.closest("[data-so-line-select-cell]")) return;
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === SO_LINES_TANSTACK_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
