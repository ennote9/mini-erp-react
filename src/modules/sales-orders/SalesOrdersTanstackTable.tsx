...
import { buildSalesOrdersTanstackColumns } from "./salesOrdersTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<SalesOrderListRow>): number | null {
...
const SALES_ORDERS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type SalesOrdersTanstackTableProps = {
  ...
  onRowClick: (row: SalesOrderListRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function SalesOrdersTanstackTable(props: SalesOrdersTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [
      SALES_ORDERS_TABLE_SELECT_COLUMN_ID,
      ...columnOrder.filter((id) => id !== SALES_ORDERS_TABLE_SELECT_COLUMN_ID),
    ],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<SalesOrderListRow, unknown> = {
      id: SALES_ORDERS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildSalesOrdersTanstackColumns({ schema, t, formatDate })];
  }, [schema, t, formatDate]);
  ...
  return (
    ...
    >
      <div
        ...
        data-sales-orders-table-scroll
      >
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === SALES_ORDERS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === SALES_ORDERS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
