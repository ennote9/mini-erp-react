...
import { buildBarcodeRegistryTanstackColumns } from "./barcodeRegistryTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<BarcodeRegistryRow>): number | null {
...
const BARCODE_REGISTRY_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type BarcodeRegistryTanstackTableProps = {
  ...
  onRowClick: (row: BarcodeRegistryRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function BarcodeRegistryTanstackTable(props: BarcodeRegistryTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [
      BARCODE_REGISTRY_TABLE_SELECT_COLUMN_ID,
      ...columnOrder.filter((id) => id !== BARCODE_REGISTRY_TABLE_SELECT_COLUMN_ID),
    ],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<BarcodeRegistryRow, unknown> = {
      id: BARCODE_REGISTRY_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [
      selectColumn,
      ...buildBarcodeRegistryTanstackColumns({
        schema,
        t,
        entryTypeLabel,
        sourceLabel,
  ...
  }, [schema, t, entryTypeLabel, sourceLabel, symbologyLabel, markdownStatusLabel]);
  ...
  return (
    ...
    >
      <div
        ...
        data-barcode-registry-table-scroll
      >
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === BARCODE_REGISTRY_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === BARCODE_REGISTRY_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
