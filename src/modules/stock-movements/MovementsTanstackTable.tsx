...
import {
  buildMovementsTanstackColumns,
  type FormatMovementDateTime,
  type FormatMovementQtyDelta,
} from "./movementsTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<StockMovementListRow>): number | null {
...
const MOVEMENTS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type MovementsTanstackTableProps = {
...
export function MovementsTanstackTable(props: MovementsTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [
      MOVEMENTS_TABLE_SELECT_COLUMN_ID,
      ...columnOrder.filter((id) => id !== MOVEMENTS_TABLE_SELECT_COLUMN_ID),
    ],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<StockMovementListRow, unknown> = {
      id: MOVEMENTS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [
      selectColumn,
      ...buildMovementsTanstackColumns({ schema, t, formatDateTimeUi, formatQtyDeltaUi, movementTypeLabel }),
    ];
  }, [schema, t, formatDateTimeUi, formatQtyDeltaUi, movementTypeLabel]);
  ...
  return (
    <div
      className={cn(
        "stock-movements-tanstack-list flex min-h-0 min-w-0 flex-1 flex-col rounded-md border border-border bg-background",
        className,
      )}
    >
      <div
        ...
        data-stock-movements-table-scroll
      >
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === MOVEMENTS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === MOVEMENTS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
