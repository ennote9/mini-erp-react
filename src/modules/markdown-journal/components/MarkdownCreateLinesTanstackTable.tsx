...
import { MARKDOWN_CREATE_TANSTACK_SELECT_COLUMN_ID } from "../markdownCreateLinesTanstackColumns";
...
export type MarkdownCreateLinesTanstackTableProps<TData> = {
  rows: TData[];
  ...
  /** When set without row selection, clicking a row invokes this (e.g. hydrate line editor). */
...
export function MarkdownCreateLinesTanstackTable<TData>(props: MarkdownCreateLinesTanstackTableProps<TData>) {
  const {
    rows,
    dataColumns,
    getRowId,
  ...
  const columns = useMemo(() => {
    ...
    const selectColumn: ColumnDef<TData, unknown> = {
      id: MARKDOWN_CREATE_TANSTACK_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            className="flex w-full items-center justify-center"
            data-markdown-create-select-cell
            onClick={(event) => event.stopPropagation()}
            onPointerDown={(event) => event.stopPropagation()}
          >
            <Checkbox
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === MARKDOWN_CREATE_TANSTACK_SELECT_COLUMN_ID;
            ...
            <tbody>
              {table.getRowModel().rows.length === 0 ? (
                ...
                table.getRowModel().rows.map((row) => {
                  ...
                  const dataClickable = Boolean(onDataRowClick) && !enableRowSelection;
                  return (
                    <tr
                      key={row.id}
                      className={cn(
                        ...
                        dataClickable && "cursor-pointer hover:bg-muted/40",
                        !enableRowSelection && !dataClickable && "cursor-default",
                      ...
                      onClick={
                        enableRowSelection
                          ? (event) => {
                              ...
                              if (target.closest("[data-markdown-create-select-cell]")) return;
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        const cellMeta = cell.column.columnDef.meta as
                          | { align?: "left" | "right" | "center"; noTruncate?: boolean }
                          | undefined;
                        const isLastBodyCell = cellIndex === visibleCells.length - 1;
                        const isSelectCell = cell.column.id === MARKDOWN_CREATE_TANSTACK_SELECT_COLUMN_ID;
                        const noTruncate = Boolean(cellMeta?.noTruncate);
                        return (
                          <td
                            key={cell.id}
                            className={cn(
                            ...
                            title={isSelectCell || noTruncate ? undefined : String(cell.getValue() ?? "")}
