...
export const MARKDOWN_JOURNAL_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

export type MarkdownJournalTanstackTableProps<T extends { id: string }> = {
  rows: T[];
  schema: MarkdownJournalTanstackColumnSchema[];
  dataColumns: ColumnDef<T, unknown>[];
  selectRowAriaCode: (row: T) => string;
...
export function MarkdownJournalTanstackTable<T extends { id: string }>(props: MarkdownJournalTanstackTableProps<T>) {
  const {
    rows,
    schema,
    dataColumns,
  ...
  const columnOrderWithSelect = useMemo(
    () => [
      MARKDOWN_JOURNAL_TABLE_SELECT_COLUMN_ID,
      ...columnOrder.filter((id) => id !== MARKDOWN_JOURNAL_TABLE_SELECT_COLUMN_ID),
    ],
    [columnOrder],
  );
  ...
  const columns = useMemo(() => {
    const selectColumn: ColumnDef<T, unknown> = {
      id: MARKDOWN_JOURNAL_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
      ...
      cell: ({ row }) => (
        ...
        >
          <Checkbox
            aria-label={t("doc.list.rowSelectRowAria", { code: selectRowAriaCode(row.original) })}
  ...
  }, [dataColumns, selectRowAriaCode, t]);
  ...
  return (
    <div
      className={cn(
        "markdown-journal-tanstack-list flex min-h-0 min-w-0 flex-1 flex-col rounded-md border border-border bg-background",
        className,
      )}
    >
      <div
        ...
        data-markdown-journal-table-scroll
      >
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === MARKDOWN_JOURNAL_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === MARKDOWN_JOURNAL_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
