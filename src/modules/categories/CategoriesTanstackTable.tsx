...
import { buildCategoriesTanstackColumns } from "./categoriesTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<CategoryListRow>): number | null {
...
const CATEGORIES_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type CategoriesTanstackTableProps = {
  ...
  onRowClick: (row: CategoryListRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function CategoriesTanstackTable(props: CategoriesTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [CATEGORIES_TABLE_SELECT_COLUMN_ID, ...columnOrder.filter((id) => id !== CATEGORIES_TABLE_SELECT_COLUMN_ID)],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<CategoryListRow, unknown> = {
      id: CATEGORIES_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildCategoriesTanstackColumns({ schema, t, formatMoney })];
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === CATEGORIES_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === CATEGORIES_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
