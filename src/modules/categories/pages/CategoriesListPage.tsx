...
function buildExportRowsFromCategories(categories: Category[]): CategoriesExportRow[] {
...
export function CategoriesListPage() {
  ...
  const getExportRowsCurrentView = useCallback((): CategoriesExportRow[] => {
    ...
    if (!api) return buildExportRowsFromCategories(filteredRows);
    const rows: Category[] = [];
    api.forEachNodeAfterFilterAndSort((rowNode) => {
      if (rowNode.data) rows.push(rowNode.data);
    });
    return buildExportRowsFromCategories(rows);
  }, [filteredRows]);
  ...
  const handleExportCurrentView = useCallback(() => {
    ...
    runExportWithSaveAs("categories.xlsx", () => buildCategoriesListXlsxBuffer(rows));
  }, [getExportRowsCurrentView, runExportWithSaveAs]);

  const handleExportSelected = useCallback(() => {
    const rows = getExportRowsSelected();
    if (rows.length === 0) return;
    runExportWithSaveAs("categories-selected.xlsx", () => buildCategoriesListXlsxBuffer(rows));
  }, [getExportRowsSelected, runExportWithSaveAs]);

  const exportSelectedDisabled = selectedCount === 0;

  const emptyTitle = hasFilter
    ? "No categories match current search or filters"
    : "No categories yet";
  const emptyHint = hasFilter
    ? "Try changing the search or filter."
    : "Create your first category to use in items.";
