...
import { buildCarriersTanstackColumns } from "./carriersTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<CarrierListRow>): number | null {
...
const CARRIERS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type CarriersTanstackTableProps = {
  rows: CarrierListRow[];
  ...
  onRowClick: (row: CarrierListRow) => void;
  onHeaderFilterClick?: (
    fieldId: string,
...
export function CarriersTanstackTable(props: CarriersTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [CARRIERS_TABLE_SELECT_COLUMN_ID, ...columnOrder.filter((id) => id !== CARRIERS_TABLE_SELECT_COLUMN_ID)],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<CarrierListRow, unknown> = {
      id: CARRIERS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
    ...
    return [selectColumn, ...buildCarriersTanstackColumns({ schema, t, formatMoney })];
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === CARRIERS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === CARRIERS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
