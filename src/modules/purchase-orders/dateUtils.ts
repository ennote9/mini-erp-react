/**
 * Purchase Order date handling: storage and HTML input type="date" use YYYY-MM-DD.
 * Year is constrained to 1900-2100 to avoid malformed display (e.g. 22005).
 */

const DATE_YYYYMMDD_REGEX = /^(\d{4})-(\d{2})-(\d{2})$/;
export const PO_DATE_MIN_YEAR = 1900;
export const PO_DATE_MAX_YEAR = 2100;
 ...
 */
export function normalizeDateForPO(value: string | undefined): string {
  ...
  if (year < PO_DATE_MIN_YEAR || year > PO_DATE_MAX_YEAR) return todayYYYYMMDD();
  return `${match[1]}-${match[2]}-${match[3]}`;
}

/** Returns error message if date is not YYYY-MM-DD or year is out of range; null if valid. */
export function validateDateForPO(value: string | undefined): string | null {
  ...
  const year = parseInt(match[1], 10);
  if (year < PO_DATE_MIN_YEAR || year > PO_DATE_MAX_YEAR)
    return `Date year must be between ${PO_DATE_MIN_YEAR} and ${PO_DATE_MAX_YEAR}.`;
  return null;
}
