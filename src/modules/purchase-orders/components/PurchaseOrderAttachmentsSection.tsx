...
export function PurchaseOrderAttachmentsSection(props: PurchaseOrderAttachmentsSectionProps) {
      ...
      try {
        ...
        setError(
          attachmentError instanceof Error
            ...
            : t("doc.po.attachments.readFailed"),
        );
      } finally {
        setBusy(false);
      }
  ...
  const handleDownload = useCallback((attachmentId: string) => {
    const attachment = purchaseOrderRepository
      .listAttachments(purchaseOrderId)
  ...
  const handleDelete = useCallback(
    (attachmentId: string) => {
      if (!window.confirm(t("doc.po.attachments.deleteConfirm"))) return;
      ...
      <CardContent className="space-y-4 p-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="space-y-1">
            <h3 className="text-sm font-semibold text-foreground">{t("doc.po.attachments.sectionTitle")}</h3>
            <p className="text-sm text-muted-foreground">{t("doc.po.attachments.sectionHint")}</p>
          </div>
          {canMutate ? (
            <Button type="button" size="sm" className="gap-1.5" onClick={handleOpenPicker} disabled={busy}>
              <Upload className="h-4 w-4" aria-hidden />
              {t("doc.po.attachments.add")}
            </Button>
          ) : null}
        </div>
        ...
        {attachments.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("doc.po.attachments.empty")}</p>
        ) : (
          <div className="overflow-x-auto rounded-md border border-border">
            <table className="w-full border-collapse text-sm leading-tight">
              <thead>
                <tr className="border-b border-border text-left text-xs text-muted-foreground">
                  <th className="px-2 py-2 font-medium">{t("doc.po.attachments.fileName")}</th>
                  <th className="px-2 py-2 font-medium">{t("doc.po.attachments.fileSize")}</th>
                  <th className="px-2 py-2 font-medium">{t("doc.po.attachments.addedAt")}</th>
                  <th className="w-[7rem] px-2 py-2 font-medium">{t("common.actions")}</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <td className="px-2 py-2 align-middle">
                      <div className="flex items-center gap-1">
                        <Button
                          ...
                          title={t("doc.po.attachments.download")}
                          aria-label={t("doc.po.attachments.download")}
                        ...
                        {canMutate ? (
                          <Button
                            ...
                            title={t("doc.po.attachments.remove")}
                            aria-label={t("doc.po.attachments.remove")}
