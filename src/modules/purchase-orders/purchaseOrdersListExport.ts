...
export type PurchaseOrdersExportRow = {
  no: number;
  number: string;
  date: string;
  supplier: string;
...
const COLUMN_HEADERS = ["№", "Number", "Date", "Supplier", "Warehouse", "Status"] as const;
const SHEET_NAME = "Purchase Orders";
const TABLE_NAME_BASE = "PurchaseOrdersTable";
...
function addSheet(workbook: Workbook, rows: PurchaseOrdersExportRow[]): void {
  ...
  const tableRows = rows.map((r) => [r.no, r.number, r.date, r.supplier, r.warehouse, r.status]);
  sheet.addTable({
  ...
  for (let c = 0; c < COLUMN_HEADERS.length; c++) {
    const valueLengths = rows.map((r) => cellLen([r.no, r.number, r.date, r.supplier, r.warehouse, r.status][c]));
...
export async function buildPurchaseOrdersListXlsxBuffer(rows: PurchaseOrdersExportRow[]): Promise<ArrayBuffer> {
