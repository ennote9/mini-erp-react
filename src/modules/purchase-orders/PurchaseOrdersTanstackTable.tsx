...
import { buildPurchaseOrdersTanstackColumns } from "./purchaseOrdersTanstackColumns";

function getColumnResizeGuideLeftPx(table: Table<PurchaseOrderListRow>): number | null {
...
const PURCHASE_ORDERS_TABLE_SELECT_COLUMN_ID = "__rowSelect__";

type PurchaseOrdersTanstackTableProps = {
...
export function PurchaseOrdersTanstackTable(props: PurchaseOrdersTanstackTableProps) {
  const {
    rows,
    schema,
    sorting,
  ...
  const columnOrderWithSelect = useMemo(
    () => [
      PURCHASE_ORDERS_TABLE_SELECT_COLUMN_ID,
      ...columnOrder.filter((id) => id !== PURCHASE_ORDERS_TABLE_SELECT_COLUMN_ID),
    ],
    [columnOrder],
  );

  const columns = useMemo(() => {
    const selectColumn: ColumnDef<PurchaseOrderListRow, unknown> = {
      id: PURCHASE_ORDERS_TABLE_SELECT_COLUMN_ID,
      size: 38,
      minSize: 36,
      maxSize: 44,
      enableSorting: false,
      ...
      cell: ({ row }) => (
        ...
        >
          <Checkbox
            aria-label={t("doc.list.rowSelectRowAria", { code: row.original.number })}
    ...
    return [selectColumn, ...buildPurchaseOrdersTanstackColumns({ schema, t, formatDate })];
  }, [schema, t, formatDate]);
  ...
  return (
    ...
    >
      <div
        ...
        data-purchase-orders-table-scroll
      >
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-border">
                  {headerGroup.headers.map((header) => {
                    ...
                    const isSelectHeader = header.column.id === PURCHASE_ORDERS_TABLE_SELECT_COLUMN_ID;
                    return (
                      <th
                        key={header.id}
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      {visibleCells.map((cell, cellIndex) => {
                        ...
                        const isSelectCell = cell.column.id === PURCHASE_ORDERS_TABLE_SELECT_COLUMN_ID;
                        return (
                          <td
                            key={cell.id}
                            className={cn(
