...
export const ruIssuesMessages: MessageTree = {
  ...
  receipt: {
    ...
    poRequired: "Нужен связанный заказ поставщику.",
    poMustBeConfirmed:
      "Связанный заказ поставщику должен быть подтверждён перед проведением.",
    warehouseRequired: "Склад обязателен.",
    warehouseInactive: "Выбранный склад неактивен.",
    linesRequired: "Нужна хотя бы одна строка.",
  ...
  save: {
    ...
    poNotFound: "Заказ поставщику не найден.",
    poOnlyDraftSave: "Сохранять можно только черновики заказов поставщику.",
    poOnlyDraftConfirm: "Подтверждать можно только черновики заказов поставщику.",
    poCancelStates:
      "Отменять можно только черновики или подтверждённые заказы поставщику.",
    poReceiptConfirmedOnly:
      "Поступление можно создать только из подтверждённого заказа поставщику.",
    poFullyReceived: "Заказ поставщику уже полностью получен (проведённые поступления).",
    poDraftReceiptExists: "Для этого заказа поставщику уже есть черновик поступления.",
    poNothingToReceive: "Для этого заказа поставщику нечего получать.",
    soUnitPriceInvalid: "В каждой строке должна быть корректная цена (число ≥ 0).",
    soNotFound: "Заказ клиента не найден.",
    soOnlyDraftSave: "Сохранять можно только черновики заказов клиента.",
    soOnlyDraftConfirm: "Подтверждать можно только черновики заказов клиента.",
    soAllocateConfirmedOnly: "Резервировать можно только подтверждённые заказы клиента.",
    soCancelStates:
      "Отменять можно только черновики или подтверждённые заказы клиента.",
    soShipmentConfirmedOnly:
      "Отгрузку можно создать только из подтверждённого заказа клиента.",
    soFullyShipped: "Заказ клиента уже полностью отгружен (проведённые отгрузки).",
    soDraftShipmentExists: "Для этого заказа клиента уже есть черновик отгрузки.",
    soNothingToShip: "Для этого заказа клиента нечего отгружать.",
