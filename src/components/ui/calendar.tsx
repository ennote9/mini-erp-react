...
function DropdownOnlySelect(props: {
  ...
  return (
    <span data-disabled={props.disabled} className={classNames[UI.DropdownRoot]}>
      <components.Select className={cssClassSelect} style={{ fontSize: "15px" }} {...selectProps}>
...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      navLayout="around"
      className={cn(
        "rdp-root px-2 py-px text-[15px]",
        "[--rdp-day-height:28px][--rdp-day-width:28px]",
        "[--rdp-day_button-height:26px][--rdp-day_button-width:26px]",
        "[--rdp-nav_button-height:26px][--rdp-nav_button-width:26px]",
        className
      )}
      classNames={{
        months: "flex flex-col gap-0",
        month: "flex flex-col gap-0",
        month_caption: "flex justify-center pt-0 pb-0 relative items-center gap-0 min-h-7",
        dropdown_root: "flex items-center gap-0",
        dropdown: "rounded-sm border border-input bg-background px-0.5 py-px leading-none text-[15px]",
        nav: "flex items-center gap-0",
        button_previous: "size-7 shrink-0 rounded border border-input bg-background flex items-center justify-center hover:bg-accent",
        button_next: "size-7 shrink-0 rounded border border-input bg-background flex items-center justify-center hover:bg-accent",
        month_grid: "w-full border-collapse",
        weekdays: "flex gap-0",
        weekday:
          "text-foreground/90 rounded w-[28px] min-w-[28px] font-normal leading-none text-center py-0",
        ...
        day_button: cn(
          "h-[26px] w-[26px] min-w-[26px] p-0 font-normal aria-selected:opacity-100 rounded-md leading-none border-2 border-transparent aria-selected:border-white",
        ...
        today: "bg-zinc-600/70 text-foreground",
        outside:
